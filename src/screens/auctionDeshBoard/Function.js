// create a modify function used for modify fetching dashboard data
export async function modifydashboardList(data) {
    var respData = { totalCount: 0, dashboardList: [] };
    if (data) {
        ...
        if (auctionList && auctionList.length > 0) {
            for (let i = 0; i < auctionList.length; i++) {
                ...
                for (let j = 0; j < auctionList[i].length; j++) {
                    ...
                    // check data auctionstateid
                    if (
                        auctionList[i][j].auctionstateid == undefined ||
                        auctionList[i][j].auctionstateid == null
                    ) {
                        modObj["status"] = "";
                    } else {
                        modObj["status"] = auctionList[i][j].auctionstateid;
                    }
                ...
                respData.dashboardList.push(modObj);
            }
        }
    }
    return respData;
...
export async function modifyCountData(data) {
    let respData = "";
    if (data) {
        let countData = data[0];
        if (countData.total.toString().length < 2) {
            respData = "0" + countData.total.toString();
        } else {
            respData = countData.total;
        }
    }
    return respData;
}
