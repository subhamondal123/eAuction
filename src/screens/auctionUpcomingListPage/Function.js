...
// create modify function used for modify fetching auctioneer upcoming list data
export async function modifyAuctioneerUpcomingList(data) {
      ...
      for (let i = 0; i < auctionList.length; i++) {
        ...
        for (let j = 0; j < auctionList[i].length; j++) {
          ...
          if (auctionList[i][j].fieldlabeldesc == "Auction Start Time") {
            modObj["startTime"] = auctionList[i][j].fieldvalue.substring(0, 5);
          }
