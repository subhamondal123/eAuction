...
class BidderListDetails extends Component {
    ...
    _apiCallRes = async () => {
        ...
        if (responseData === false) {
            ...
            if (responseData.status === ErrorCode.ERROR.ERROR_CODE.SUCCESS) {
                ...
                console.log("responseeeee---------", JSON.stringify(auctioneerLiveListDetails))

                this.setState({
                    auctionDetailsArr: auctioneerLiveListDetails.auctioneerLiveList,
                    auctionDetails: auctioneerLiveListDetails
                })
    ...
    _activityLogApiRes = async () => {
        ...
        console.log("---_activityLogApiRes---", JSON.stringify(responseData))
                        ...
                        <View style={styles.BoxMarginSec}>
                            ...
                            <View style={styles.BoxMarginSubSec}>
                                <View style={styles.columnSec}>
                                    <Text style={styles.auctionText}>{this.state.auctionDetails.auctionId}</Text>
                                </View>
                        ...
                        <View>
                            <View style={{ backgroundColor: "#EBEBEB" }}>
                                <View style={{ marginHorizontal: '3%' }}>
                                    ...
                                    <View style={[styles.auctionSec, { flexWrap: "wrap", flex: 1 }]}>
                                        {this.state.auctionDetailsArr.map((item, key) => (
                                            <View key={key}>
                                                <View style={[styles.typeSecArr, { alignItems: (key % 2 == 0) ? "flex-start" : "flex-end" }]}>
                                                    {/* {item.fielddesc == "actualauctionenddatetime" || item.fielddesc == "auctionenddatetime" || item.fielddesc == "auctionstartdatetime" || item.fielddesc == "deliveryinstruction" ? null : */}
                                                    <>
                                                        <Text style={styles.productUnitText}>{item.fieldlabeldesc}</Text>
                                                        <Text style={styles.valueText}>{item.fieldvalue}</Text>
                                                        {item.fielddesc == "auctiondescfile" ?
                                                            <TouchableOpacity activeOpacity={0.9} onPress={() => this.downloadDoc(item.fieldvalue)}>
                                                                <Text style={styles.productUnitText}>{item.fieldvalue}</Text>
                        ...
                        <View style={styles.space}>
                            ...
                            <ScrollView showsHorizontalScrollIndicator={false}
                                ...
                                <View style={styles.activityLogSec}>
                                    ...
                                    {this.state.activityLoader ? <ActivityIndicator /> :
