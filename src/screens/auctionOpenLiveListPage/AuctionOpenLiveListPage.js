...
import { modifyAuctioneerLiveList, modifyAuctioneerOpenList } from './Function';
import { CustomStyle } from '../style';
import socketServices from '../../services/api/models/socketService';

// this is auctioneer open list page 
class AuctioneerOpenListPage extends Component {
    constructor(props) {
        super(props);
        this.state = {
            openAuctionList: [],
    ...
    // for fetching api call responce 
    _apiCallRes = async () => {
        let userData = await storageDataModification.userCredential({}, "get");
    ...
    _onOpenDetails = (item, key) => {
        this.props.navigation.navigate("AuctioneerListDetails", { data: item, status: 5, key: key.toString().length > 1 ? key + 1 : "0" + (key + 1), onReload: this.onRefresh });
...
export default connect(mapStateToProps, mapDispatchToProps)(AuctioneerOpenListPage);
