...
export function getAllDateInfoInString(date) {
    try {
        let resDate = { "year": "", "month": "", "day": "", "hour": "", "minutes": "", "second": "", "ampm": "" };
        if (date) {
            const currentDate = new Date(date);
            ...
            resDate = { "year": year, "month": month, "day": day, "hour": hours, "minutes": minutes, "second": secound, "ampm": ampm }
        }
        return resDate;
    } catch (err) {
        console.log(err)
...
// get currentMonth first date and last date in time stamp
export function getCurrentMonthFirstAndLastDate() {
    try {
        let date = new Date(),
            y = date.getFullYear(),
            m = date.getMonth() + 1,
            lastdays = new Date(date.getFullYear(), date.getMonth() + 1, 0);
        days = lastdays.getDate()
        hour = date.getHours(),
            minute = date.getMinutes(),
            second = date.getSeconds();
        let firstDay
        if (Number(m) < 10) {
            firstDay = y + '-' + 0 + m + '-' + '01' + 'T00:00:00.000Z'
        } else {
            firstDay = y + '-' + m + '-' + '01' + 'T00:00:00.000Z'
        }

        let lastDay
        if (Number(m) < 10) {
            lastDay = y + '-' + 0 + m + '-' + days + 'T23:59:59.999Z'
        } else {
            lastDay = y + '-' + m + '-' + days + 'T23:59:59.999Z'
        }
        // let lastDay = new Date(y, m + 1, days);
        let currentMonthFirstDate = Date.parse(firstDay);
        let currentMonthLastDate = Date.parse(lastDay);
        return { firstDate: currentMonthFirstDate, lastDate: currentMonthLastDate };
    } catch (err) {
        console.log(err)
    }
}
...
export function getTommorowDate() {
    try {
        ...
        let tommorowDate = Date.parse(tomorrow);
        return tommorowDate;
    } catch (err) {
        console.log(err)
    }
...
// format 24 hrs - HH:MM:SS
export function TimeFormat24Hrs(date) {
    try {
